      ...
      <div class="container-fluid">
        <div class="row">
         ...
         $fechasqlfin4 = "$fechasqlfinxx[year]-$fechasqlfinxx[mon]-$fechasqlfinxx[mday]";
          ...
          ?>
            ...
            <div class="card">
              ...
              <form align="right" action ="" method="margin-top">
                  <input type="date" id="fechadata4ini" name="fechadata4ini" min="2015-11-21" max="2025-12-31"></input> 
          ...
          <?php
      $sqltopsuc="SELECT service.name,servicesubcategory.name, COUNT(ticket.id) total  FROM ticket 
      INNER JOIN ticket_incident ON ticket.id = ticket_incident.id
      INNER JOIN servicesubcategory  ON ticket_incident.servicesubcategory_id = servicesubcategory.id
      INNER JOIN service ON ticket_incident.service_id = service.id
      WHERE start_date BETWEEN CONCAT('$fechasqlini4', ' 00:00:00') AND CONCAT('$fechasqlfin4', ' 23:59:00')
      AND team_id IN ('4')
      group BY servicesubcategory.name
      ORDER BY total DESC      
  ";

  
$resul=mysqli_query($conitop,$sqltopsuc);
 mysqli_close($conitop);


for($mostrar = 1;$mostrar<=10;$mostrar++){
